...
const navSections = [
  { label: "Command", items: [{ href: "/dashboard", label: "Overview", icon: Home },{ href: "/dashboard/projects", label: "Projects", icon: LayoutDashboard },{ href: "/dashboard/workflow", label: "Workflow", icon: Calendar }]},
  ...
  { label: "Execute", items: [{ href: "/dashboard/audit", label: "Technical Audit", icon: Shield },{ href: "/dashboard/schema", label: "Schema Markup", icon: Braces },{ href: "/dashboard/brief", label: "Content Brief", icon: Lightbulb },{ href: "/dashboard/content", label: "Content Studio", icon: FileText },{ href: "/dashboard/programmatic", label: "Programmatic", icon: Grid3x3 },{ href: "/dashboard/links", label: "Link Building", icon: Link2 },{ href: "/dashboard/reports", label: "Reports", icon: ClipboardList }]},
    ...
    <aside className="w-64 h-screen fixed left-0 top-0 z-40 flex flex-col" style={{ background: "var(--bg-primary)", borderRight: "1px solid var(--border)" }}>
      <div className="px-6 py-6" style={{ borderBottom: "1px solid var(--border)" }}>
        <Link href="/dashboard" className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center font-serif text-base font-bold" style={{ background: "linear-gradient(135deg, #8B5CF6, #EC4899)", color: "#ffffff" }}>OR</div>
          ...
          <div key={section.label}>
            ...
            <div className="space-y-0.5">
              {section.items.map((item) => {
                ...
                return (<Link key={item.href} href={item.href} className="flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] font-sans font-medium transition-all duration-200" style={{ color: active ? "var(--text-primary)" : "var(--text-muted)", background: active ? "rgba(139,92,246,0.10)" : "transparent", borderLeft: active ? "2px solid #8B5CF6" : "2px solid transparent" }}>
